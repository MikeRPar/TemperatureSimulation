...
//pickPhysicalDevice helper function
...
void GraphicsEngine::createLogicalDevice()
{
	...
	//Extension details, device specific
	createInfo.enabledExtensionCount = static_cast<uint32_t>(deviceExtensions.size());
	createInfo.ppEnabledExtensionNames = deviceExtensions.data();

	//Validation layers are seperate by instance and device in older vulkan implementations
	//Newer Vulkan implementations change this and ignore the device enabledLayerCount and ppEnabledLayerNames
	//For compatibility its safe to set these anyway
	if (enableValidationLayers)
	{
		createInfo.enabledLayerCount = static_cast<uint32_t>(validationLayers.size());
		createInfo.ppEnabledLayerNames = validationLayers.data();
	...
	{
		throw std::runtime_error("Failed Logical Device Creation!");
	}
...
void GraphicsEngine::createTextureSampler()
{
	...
	/* Settings to disable anisotropic filtering, which is a potentially non supported extensions (usually is though)
...
void GraphicsEngine::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
	...
	vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
		...
		//vkCmdDraw(commandBuffer, static_cast<uint32_t>(vertices.size()), 1, 0, 0); //Draw without index buffer
...
void GraphicsEngine::initWindow()
{
	...
	glfwSetWindowUserPointer(window, this); //Give GLFW a pointer to this class
